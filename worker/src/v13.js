...
const VIEWER_SOURCE = "static-cdn";
...
async function handleManifest(request, env, ctx, submissionId, outputId) {
  ...
  const [submissions, status, session] = await Promise.all([
    loadStaticJson(SUBMISSIONS_PATH, DATA_CACHE_SECONDS, env, ctx),
    loadStaticJson(STATUS_PATH, STATUS_CACHE_SECONDS, env, ctx).catch(() => ({ phases: [] })),
...
async function handleFile(request, env, ctx, { submissionId, fileId, download, outputId }) {
  const submissions = await loadStaticJson(SUBMISSIONS_PATH, DATA_CACHE_SECONDS, env, ctx);
  const context = findSubmissionRecord(submissions, submissionId, outputId);
  const record = context.record;
  validateRecord(record);
  ...
  const upstream = await loadStaticFile(file.path, {
    env,
    ctx,
    range,
  ...
  const headers = new Headers({
    ...
    "X-Viewer-Source": VIEWER_SOURCE,
    "X-Viewer-Upstream": upstream.headers.get("X-VCAT-Upstream-Source") || "cache",
    "X-GitHub-User-Token-Used": "false"
  });
  ...
  if (request.method === "HEAD") return corsResponse(new Response(null, { status: upstream.status, headers }), request, env);
  return corsResponse(new Response(upstream.body, { status: upstream.status, headers }), request, env);
}

async function loadStaticJson(path, cacheSeconds, env, ctx) {
  const response = await loadStaticResource(path, { env, ctx, cacheSeconds, category: "data" });
  try {
    return await response.json();
  } catch {
    throw httpError(502, `${path} 파일 형식이 올바르지 않습니다.`, "STATIC_JSON_INVALID");
  }
}

async function loadStaticFile(path, { env, ctx, range = null, cacheSeconds = 0 }) {
  return loadStaticResource(path, { env, ctx, cacheSeconds, category: "file", range });
}

async function loadStaticResource(path, { env, ctx, cacheSeconds, category, range = null }) {
  ...
  const headers = new Headers({
    Accept: "*/*",
    "User-Agent": "vcat-dashboard-static-reader-v13"
  });
  if (range) headers.set("Range", range);

  let lastResponse = null;
  for (const candidate of sourceCandidates(env, path)) {
    ...
    normalizedHeaders.set("X-VCAT-Upstream-Source", candidate.source);
    ...
    if (cache && cacheSeconds > 0 && !range) {
      const write = cache.put(key, normalized.clone());
  ...
  throw staticHttpError(lastResponse, path);
}

function sourceCandidates(env, path) {
  const repository = String(env.REPOSITORY || "");
  const [owner, repo] = repository.split("/");
  const encodedPath = encodePath(path);
  const candidates = [];

  if (path.startsWith("docs/") && owner && repo) {
    const relativePath = encodePath(path.slice("docs/".length));
    const frontendBase = String(env.FRONTEND_URL || `https://${owner}.github.io/${repo}/`).replace(/\/?$/, "/");
    candidates.push({ source: "github-pages", url: new URL(relativePath, frontendBase).toString() });
  }

  candidates.push({
    source: "jsdelivr",
    url: `https://cdn.jsdelivr.net/gh/${repository}@${DEFAULT_BRANCH}/${encodedPath}`
  });
  candidates.push({
    source: "github-raw-fallback",
    url: `https://raw.githubusercontent.com/${repository}/${DEFAULT_BRANCH}/${encodedPath}`
  });
  return candidates;
}
...
function staticHttpError(response, path) {
  if (!response) return httpError(502, "정적 파일 서비스를 사용할 수 없습니다.", "STATIC_UPSTREAM_UNAVAILABLE");
  const remaining = response.headers.get("X-RateLimit-Remaining");
  const reset = response.headers.get("X-RateLimit-Reset");
  const retryAfter = response.headers.get("Retry-After");
  if (response.status === 429 || (response.status === 403 && remaining === "0")) {
    const retryAt = reset ? new Date(Number(reset) * 1000).toISOString() : null;
    const error = httpError(503, "정적 파일 조회가 일시적으로 제한되었습니다. 잠시 후 다시 시도해 주세요.", "STATIC_RATE_LIMIT");
    error.retryAt = retryAt;
    error.retryAfter = retryAfter ? Number(retryAfter) : null;
    return error;
  }
  if (response.status === 404) return httpError(404, `저장소 파일을 찾지 못했습니다: ${path}`, "STATIC_NOT_FOUND");
  return httpError(502, "정적 파일 조회가 일시적으로 지연되고 있습니다.", "STATIC_UPSTREAM_ERROR");
}
...
function readErrorResponse(request, env, error, fallback) {
  ...
  const payload = {
    ...
    code: error.code || "STATIC_READ_ERROR",
...
function corsResponse(response, request, env) {
  ...
  headers.set("Access-Control-Expose-Headers", "Content-Disposition, Content-Length, Content-Range, Accept-Ranges, ETag, Last-Modified, X-Viewer-Source, X-Viewer-Upstream, X-GitHub-User-Token-Used");
